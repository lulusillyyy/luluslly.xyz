---
title: Designing a Corne-style Keyboard Layout Using Kanata 
date: 2026-05-15
categories:
    - keyboard
    - computing
    - ergonomics
tags:
    - keyboard
    - kanata
slug: kanata-config
draft: true
---
As of late, I have been working on keeping up-to-date repositories of my various configuration files, which can now be viewed on my [Forgejo instance](https://basedprinter.xyz/lulu/). One of the things I have put a decent amount of work into has been my daily-use keyboard layout. Seeing as I've fallen hook, line, and sinker for the ergonomic keyboard craze, I've come up with a relatively comfortable setup using the [Kanata](https://github.com/jtroo/kanata) program.

<img src="/images/kanata-icon.svg" alt="A white letter K imprinted on a pastel pink keyboard key cap Kanata logo">

Kanata -- in simple terms -- is a **keyboard remapper**. You can configure your keyboard to execute certain commands when a specific sequence of keys are pressed, do one-to-one remaps, and set up layers to allow you to access keys that would otherwise be a pain in the ass to enter traditionally.

As I've been privy to Kanata for some months now, my configuration has changed quite a bit as I've learned the extent to which I can utilize layering and key chords. As a matter of fact, within the past few months, I have recently completed what should be my "endgame" configuration. [Feel free to take a look at the Windows iteration!](https://basedprinter.xyz/lulu/kanata-win)

To explain my layout as briefly as I can, I'm rocking a 34-key layout, with multiple layers and a couple other binds to make all of the essential keys accessible while cutting down on the number of physical keys I need to press across the entire keyboard. 
